...
If the resource is not supported by the [DAB syntax](https://docs.databricks.com/aws/en/dev-tools/bundles/resources#apps) (e.g. UC tables) then it will need to be injected in the [deploy action](./.github/workflows/deploy.yml) — add the entry to `app/uc-extras.json`.
